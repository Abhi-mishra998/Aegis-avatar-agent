...
const envTokenIsSet = Boolean(
  (import.meta.env.VITE_TAVUS_API_KEY as string | undefined)?.trim(),
);
...
export const Intro: React.FC = () => {
  ...
  // If the API key arrived from .env.local, skip the manual unlock screen.
  useEffect(() => {
    if (envTokenIsSet && token) {
      setScreenState({ currentScreen: "instructions" });
    }
  }, [token, setScreenState]);
    ...
    <AnimatedWrapper>
      <div className="flex size-full flex-col items-center justify-center">
        ...
        <div className="relative z-10 flex flex-col items-center gap-2 py-4 px-4 rounded-xl border border-[rgba(255,255,255,0.2)]" 
          ...
          <h1 className="text-2xl font-bold text-white mb-0" style={{ fontFamily: 'Source Code Pro, monospace' }}>Aegis</h1>
          <p className="text-sm text-white/70 mb-1">Talk to Abhishek — 24/7</p>
